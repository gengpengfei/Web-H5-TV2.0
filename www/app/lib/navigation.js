...
define(['jquery'], function($) {
    ...
    function gotoLeaveFor(sectionId, direction) {
        if (_sections[sectionId].leaveFor &&
            ...
            if (typeof next === 'string' && next.charAt(0) == '@') {
                nextSectionId = selector.substr(1);
            }
        ...
        if (!direction) {
            if (evt.keyCode == 13) {
                ...
                if (currentFocusedElement && getSectionId(currentFocusedElement)) {
                    var downFocusProperties = {
                        previousElement: currentFocusedElement,
                        sectionId: getSectionId(currentFocusedElement),
                        direction: "enter-down",
                        native: false
                    };
                    if (!fireEvent(currentFocusedElement, 'enter-down', downFocusProperties)) {
                        return preventDefault();
                    }
                }
            }

            if (evt.keyCode == 147 || evt.keyCode == 219 || evt.keyCode == 27 || evt.keyCode == 8 || evt.keyCode == 461) {
                currentFocusedElement = getCurrentFocusedElement();
                if (!currentFocusedElement) {
                    if (_lastSectionId) {
                        currentFocusedElement = getSectionLastFocusedElement(_lastSectionId);
                ...
                if (currentFocusedElement && getSectionId(currentFocusedElement)) {
                    var upFocusProperties = {
                        previousElement: currentFocusedElement,
                        sectionId: getSectionId(currentFocusedElement),
                        direction: "back",
                        native: false
                    };
                    if (!fireEvent(currentFocusedElement, 'enter-back', upFocusProperties)) {
                        return preventDefault();
                    }
                }
            }
    ...
    function onClick(evt) {
        ...
        var checkFocus = target.hasAttribute('tabindex');
        var currentFocusedElement = getCurrentFocusedElement();
        if (checkFocus && currentFocusedElement && getSectionId(currentFocusedElement)) {
            if (!fireEvent(currentFocusedElement, 'enter-click')) {
                evt.preventDefault();
                evt.stopPropagation();
            }
        }
    ...
    function onFocus(evt) {
        ...
        if (target !== window && target !== document && _sectionCount && !_duringFocusChange) {
            ...
            //-- 兼容android 使用 requestFocus 后 焦点在 无效的div上
            var checkFocus = target.getAttribute('tabindex');
            if (_sectionCount && !checkFocus) {
                if (_lastSectionId) {
                    target = getSectionLastFocusedElement(_lastSectionId);
                    target.focus();
                } else {
                    focusSection();
                    evt.preventDefault();
        ...
        //-- 兼容android 使用 requestFocus 后 焦点在document上
    ...
    var SpatialNavigation = {
        ...
        makeFocusable: function(sectionId) {
            ...
            if (sectionId) {
                if (_sections[sectionId]) {
                    doMakeFocusable(_sections[sectionId], sectionId);
                } else {
                    throw new Error('Section "' + sectionId + '" doesn\'t exist!');
                }
            } else {
                for (var id in _sections) {
                    doMakeFocusable(_sections[id], id);
                }
            }
        },
        ...
        //-- 获取上一个焦点所在的sectionId
        getPrevSectionId: function() {
            return _prevSectionId;
        },
        //-- 获取最后焦点所在的sectionId
        getLastSectionId: function() {
            return _lastSectionId;
        },
        //-- 获取sectionId对应的最后焦点所在
        getLastFocus: function(sectionId) {
            return getSectionLastFocusedElement(sectionId);
        },
        //-- 根据 elem 获取对应的 sectionId
        getSectionId: function(elem) {
            return getSectionId(elem);
        }
    };
